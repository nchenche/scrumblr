...
const db = {
    createUser: function(username, email, password) {
        ...
        redisClient.hexists("users", username, async (err, userExists) => {
            if (err) {
            ...
            redisClient.hexists("emails", email, async (err, emailExists) => {
                if (err) {
                ...
                redisClient.hset("users", username, hashedPassword, (err, userRes) => {
                    ...
                    redisClient.hset("emails", email, username, (err, emailRes) => {
                        ...
                        console.log('User and email stored successfully.', userRes, emailRes);
                        return true;
                    });
                });
            });
