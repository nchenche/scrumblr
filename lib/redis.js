...
const db = {
	...
	getUserRooms: function (user, callback) {
		...
		redisClient.smembers(keyParticipant, (error, participantMembers) => {
			...
			redisClient.smembers(keyOwner, (error, ownerMembers) => {
				...
				// Prepare data for owner rooms with password fetching
				const ownerPromises = ownerMembers.map(room => {
					return new Promise(resolve => {
						this._getPassword(room, password => {
							resolve({ room: room, role: 'owner', password: Buffer.from(password, 'base64').toString() || '' });
						});
					});
				});

				// Prepare data for participant rooms (no passwords)
				const participantData = participantMembers.map(room => ({ room: room, role: 'participant', password: '' }));

				// Combine all promises and process them together
				Promise.all(ownerPromises).then(ownerData => {
					const allRoomsData = ownerData.concat(participantData);
					return callback({ success: true, message: "Successfully retrieved rooms for user.", data: allRoomsData });
				}).catch(error => {
					console.error('Error processing rooms data:', error);
					return callback({ success: false, message: "Error processing rooms data.", error: error });
				});
			});
		});
	},
	deleteUserRoom: async function(user, room, callback) {
		...
		try {
			...
			// delete all room related redis keys (keyRoom, keyOwner, keyParticipant) 
			const keyParticipant = `user:${user}:participant`;
	...
	_getPassword: function (room, callback) {
