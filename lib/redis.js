...
const db = {
	...
	storeToken: function (user, expiresIn, callback) {
		...
		redisClient.set(key, token, 'EX', expiresIn, (err, response) => {
			...
			callback({ success: true, message: `Token stored: ${response}`, token: response });
		});
	},
	resetPassword: function (username, newPassword, res) {
		// Hash the new password
		bcrypt.hash(newPassword, saltRounds, (err, hashedPassword) => {
			if (err) {
				console.error('Error hashing password:', err);
				return res.status(500).json({ success: false, message: 'Failed to hash password', error: err });
			}
			...
			redisClient.hset("users", username, hashedPassword, (err, userRes) => {
				if (err) {
					...
					return res.status(500).json({ success: false, message: 'Error storing new password', error: err });
				}
	
				// Clear the reset token
				redisClient.del(`reset_token:${username}`, (err, delRes) => {
					if (err) console.error('Error deleting reset token:', err);
	...
	checkToken: function(username) {
		// Clear the reset token
		redisClient.get(`reset_token:${username}`, (err, delRes) => {
			if (err) console.error('Error deleting reset token:', err);
			console.error('Error clearing reset token:', err);
		});
