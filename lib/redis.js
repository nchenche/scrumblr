const redis = require("redis");
const async = require("async");
const sets = require('simplesets');
const bcrypt = require('bcrypt');
const crypto = require('crypto');

const { promisify } = require('util');

/**************
 GET CONFIG VARS
**************/
const { config } = require('../config/');
const { existsSync } = require("fs");

console.log('Redis database configuration:', config.redis);


var redisClient = null;
var isDemoRoomInit = false;
const saltRounds = 10;
var REDIS_PREFIX = config.redis.prefix;

// Promisify bcrypt methods
const hashAsync = promisify(bcrypt.hash);
const genSaltAsync = promisify(bcrypt.genSalt);


(function initializeRedisClient() {
	if (!redisClient) {
		console.log('Initializing Redis client...');
		redisClient = redis.createClient({
			url: config.redis.url,
			legacyMode: true
		});

		redisClient.on("connect", () => {
			console.log("Redis client connected");
			if (!isDemoRoomInit) {
				// cleanAndInitializeDemoRoom();
				isDemoRoomInit = true;
			}
		});

		redisClient.on("error", (err) => {
			console.error("Redis client error", err);
		});

	}
})();

function promisifyMethods(client) {
	// List of Redis methods to promisify
	const redisMethods = [
		'sadd',
		'sismember',
		'get',
		'set',
		'del',
		'srem',
		'smembers',
		'hexists',
		'hget',
		'hgetall',
		'hset',
		'hdel',
		'exists',
	];

	return redisMethods.reduce((acc, method) => {
		acc[`${method}Async`] = promisify(client[method]).bind(client);
		return acc;
	}, {});
}

const asyncMethods = promisifyMethods(redisClient);

// Destructure the methods
const {
	saddAsync,
	sismemberAsync,
	getAsync,
	setAsync,
	delAsync,
	sremAsync,
	smembersAsync,
	hexistsAsync,
	hgetAsync,
	hgetallAsync,
	hsetAsync,
	hdelAsync,
	existsAsync,
} = asyncMethods;


const db = {
	createUser: async (username, email, password) => {
		try {
			// Hash the password
			const hashedPassword = await hashAsync(password, saltRounds);

			// Store the new username and hashed password
			const userRes = await hsetAsync("users", username, hashedPassword);

			// Store the email associated with the user
			const emailRes = await hsetAsync("emails", email, username);

			// Add user as participant in all public rooms
			await db.addUserToPublicRooms(username);

			return {
				success: true,
				message: 'User successfully created.',
				user: userRes,
			};
		} catch (err) {
			console.error('Error during user creation:', err);
			return {
				success: false,
				message: 'Error during user creation.',
				error: err.message || err,
			};
		}
	},
	createAdminUser: async (username, email, password) => {
		try {
			let message = 'User and email stored successfully';

			// Check if emails/username couple already exists
			const prevUser = await hgetAsync('emails', email);
			if (prevUser && prevUser == username) {
				message = `Admin ${username} already exists.`;
				return { success: true, message: message };
			}

			const [responseEmails, adminAdd] = await Promise.all([
				hsetAsync('emails', email, username),
				saddAsync('admin:users', username),
			]);

			// Check if username already exists
			const prevUsername = await hexistsAsync('users', username);
			if (prevUsername == 1) {
				message = `User ${username} already exists, please choose another one`;
				return { success: false, message: message };
			}
			// Generate a salt
			const salt = await genSaltAsync(saltRounds);

			// Hash the password
			const hashedPassword = await hashAsync(password, salt);

			// Store the new username and hashed password
			const responseUsers = await hsetAsync('users', username, hashedPassword);

			const responses = {
				isCredentialStored: responseUsers,
				isEmailStored: responseEmails,
				isAdminAdd: adminAdd
			};

			return { success: true, message: message, status: responses };
		} catch (err) {
			console.error('Error:', err);

			return {
				success: false,
				message: 'An error occurred',
				error: err.message || err,
			};
		}
	},
	removeUser: async (user) => {
		try {
			// delete user in emails
			await db.deleteUserEmail(user);

			// delete user in users
			const temp = await hdelAsync("users", user);

			// delete all rooms-related data for each rooms created by the user
			const ownedRooms = await db.getOwnedRooms(user);
			if (!ownedRooms.data) {
				const message = `No rooms found to be created by user ${user}... Deletion process done.`
				return { success: true, message: message, params: { user } }
			}

			// Delete all related rooms
			await Promise.all(
				ownedRooms.data.map((room) => db.deleteUserRoom(user, room))
			);

			// Ensure removal of user from admin:users
			await sremAsync("admin:users", user);

			const message = `User ${user} and its related data successfully deleted.`
			return { success: true, message: message, params: { user } };

		} catch (error) {
			console.error(`Error in retrieving rooms created by user ${user}: ${error}`);
		}
	},
	getOwnedRooms: async (user) => {
		try {
			const key = `user:${user}:owner`;

			const ownedRooms = await smembersAsync(key);
			if (!ownedRooms) {
				console.log(`No rooms found to be created by user ${user}`);
			}

			console.log(`Successfully retrieved rooms created by user ${user}`);
			return { success: true, data: ownedRooms, params: { user } }

		} catch (error) {
			console.error(`Error in retrieving rooms created by user ${user}: ${error}`);
		}
	},
	getEmailByUser: async (user) => {
		try {
			const data = await hgetallAsync('emails');

			// Find the email by iterating over the hash
			const email = Object.keys(data).find(key => data[key] === user);

			return { success: true, data: email, params: { user } };

		} catch (error) {
			console.error(`Error in retrieving rooms created by user ${user}: ${error}`);
		}
	},
	deleteUserEmail: async function (user) {
		try {
			const responseGetEmail = await db.getEmailByUser(user);

			if (responseGetEmail?.data) {
				const responseEmailDel = await hdelAsync("emails", responseGetEmail.data);
				return 1
			}
			return 0
		} catch (error) {
			console.error(`Error in emails for user ${user}: ${error}`);
		}
	},
	addUserToPublicRooms: async (user) => {
		try {
			const publicRooms = await smembersAsync("rooms:public");
			if (!publicRooms) {
				return {
					success: true,
					message: `No public rooms found.`,
					publicRooms: publicRooms
				}
			}

			let isAdded = null;
			publicRooms.forEach( 
				async (room) => {
					isAdded = await db.addRoomToUserAsParticipant(user, room);
				}
			)

			return {
				success: true,
				message: `User successfully added as participant in public rooms`,
				publicRooms: publicRooms,
				isAdded: isAdded
			}
		} catch (error) {
			console.error(`Error in adding user ${user} to public rooms: ${error}`);
		}
	},
	createPublicRoom: async (adminUser, room) => {
		try {
			// Checks if user is an admin user
			const isUserAdmin = await sismemberAsync("admin:users", adminUser);
			if (isUserAdmin == 0) {
				return {
					success: false,
					error: `Failure... Public room creation is only allowed from admin users only.`
				}
			}

			// Creates room 
			const responseStoreRoom = await db.storeRoomDetails(room, adminUser);
			console.log(responseStoreRoom);

			// Add room to dedicated public rooms set
			await saddAsync("rooms:public", room);

			// Get all scrumblr users to add them as participant in the public room
			const scrumblrUsers = await db.getAllUsers();

			scrumblrUsers.result.users
				.filter(user => user != adminUser)
				.forEach(async (user) => {
					console.log(user);
					await db.addRoomToUserAsParticipant(user, room);
				})

			const roomData = await db._getRoomData(room);

			const responses = {
				isUserAdmin: isUserAdmin,
				roomStorage: responseStoreRoom,
			}
			return { success: true, data: roomData, responses: responses };

		} catch (error) {
			console.error(`Error in creating public room ${room}: ${error}`);
			return { success: true, data: null };
		}
	},
	authenticateUser: function (username, password, callback) {
		redisClient.hget("users", username, (err, hashedPassword) => {
			if (err) {
				console.error('Error fetching user:', err);
				callback({ success: false, message: 'Authentication process failed', error: err });  // Pass the error in the callback
				return;
			}

			if (!hashedPassword) {
				console.log('User does not exist.');
				callback({ success: false, message: 'User does not exist.', type: "username" });  // User not found
				return;
			}

			bcrypt.compare(password, hashedPassword, (err, match) => {
				if (err) {
					console.error('Error comparing password:', err);
					callback({ success: false, message: 'Authentication process failed', error: err });  // Pass the error in the callback
					return;
				}

				if (match) {
					const sessionId = `session:${username}:${new Date().getTime()}`;
					redisClient.setex(sessionId, 3600 * 6, JSON.stringify(username), (err, reply) => {
						if (err) {
							console.error('Error storing session:', err);
							callback({ success: false, message: 'Session storage failed', error: err });
						}
						console.log('Session storage successfull:', sessionId);
					});
					console.log('Login successful for:', username);
					callback({ success: true, message: 'Login successful.', user: username, session: sessionId });
				} else {
					console.log('Invalid password');
					callback({ success: false, message: 'Invalid password', type: "password" });  // Password does not match
				}
			});
		});
	},
	getUserBySession: function (sessionId, callback) {
		redisClient.get(sessionId, (err, result) => {
			if (err || result === null) {
				const message = `Invalid or expired session: ${err}`;
				callback({ success: false, message: message });
				return;
			}
			const message = "Successfull user retrieval from session";
			const username = JSON.parse(result);
			const response = { success: true, message: message, username: username };
			callback(response);
		});
	},
	getEmailFromUser: function (username, callback) {
		redisClient.hgetall("emails", (err, emails) => {
			if (err) {
				const message = `Error retrieving all emails: ${err}`;
				console.error(message);
				return;
			}
			const email = Object.keys(emails).find(key => emails[key] === username);
			if (email) {
				const message = `Email for username ${username} is ${email}`;
				callback({ success: true, message: message, email: email });
				return
			} else {
				const message = `Username ${username} not found.`;
				console.log(message);
				callback({ success: false, message: message });
			}
		});
	},
	getAllUsers: async function () {
		try {
			const key = `emails`;
			const users = await hgetallAsync(key);

			const data = { emails: [], users: [] };
			// const data = {users: []};

			if (users) {
				console.log(`Successfully retrieval of all users from key: ${key}`);

				for (const [email, user] of Object.entries(users)) {
					// data.users.push({[user]: {email: email}});
					data.emails.push(email);
					data.users.push(user);
				}
			} else {
				console.log(`No users found in key: ${key}`);
			}

			return { result: data };
		} catch (error) {
			console.error(`Error retrieving users in ${key} key: ${error}`);
		}
	},
	storeToken: async (user, expiresIn, callback) => {
		const key = `reset_token:${user}`;  // Key format could be anything that uniquely identifies the purpose
		const token = generateToken();

		try {
			// Store the token with expiration time
			const response = await setAsync(key, token, 'EX', expiresIn);

			return {
				success: true,
				message: `Token stored: ${response}`,
				token: token,
			};
		} catch (err) {
			const message = `Error storing token: ${err}`;
			console.error(message);
			return {
				success: false,
				message,
				error: err
			};
		}
	},
	resetPassword: function (username, newPassword, callback) {
		// Hash the new password
		bcrypt.hash(newPassword, saltRounds, (err, hashedPassword) => {
			if (err) {
				console.error('Error hashing password:', err);
				return callback({ success: false, message: 'Failed to hash password', error: err });
			}

			// Store the new hashed password
			redisClient.hset("users", username, hashedPassword, (err, userRes) => {
				if (err) {
					console.error('Error storing new password:', err);
					return callback({ success: false, message: 'Error storing new password', error: err });
				}

				// Clear the reset token
				redisClient.del(`reset_token:${username}`, (err, delRes) => {
					if (err) {
						console.error('Error deleting reset token:', err);
						return callback({ success: false, message: 'Error deleting reset token', error: err });
					}
					return callback({ success: true, message: 'Password reset successfully' });
				});
			});
		});
	},
	checkToken: function (username, token, callback) {
		const key = `reset_token:${username}`;

		redisClient.get(key, (err, result) => {
			if (err) {
				console.error('Error accessing token:', err);
				return callback({ success: false, message: 'Error accessing token.', error: err });
			}

			if (result === token) {
				return callback({ success: true, message: "Token is valid." });
			} else {
				return callback({ success: false, message: "Invalid or expired token." });
			}
		});
	},
	_getRoomData: async function (room) {
		const keyRoom = `room:${room}`;  // -> "{\"owner\":\"john\",\"participants\":[]}"

		try {
			const roomDataRaw = await getAsync(keyRoom);
			const roomData = roomDataRaw ? JSON.parse(roomDataRaw) : {};

			return roomData;
		} catch (error) {
			console.error(`Error while getting room data members from key ${keyRoom}: ${error}`);
		}
	},
	_getRoomOwner: async (room) => {
		try {
			const roomData = await this._getRoomData(room);
			const owner = roomData ? roomData.owner : "";

			return owner;
		} catch (error) {
			console.error(`Error while getting room ownership: ${error}`);
		}
	},
	storeRoomDetails: async (room, user) => {
		/* Creates room-related data
		
		The creation of a room implies:
			- a hash to store room members info. If the room does not exist, user is set as owner
				- key: room:${room}
				- value: "{\"owner\":\"john\",\"participants\":[]}"
			- a set to store rooms owned by the user
				- key: user:${user}:owner
				- value: room1, room2....
	
		Warning: at this step, if user is not the owner, he will not be stored as a participant
		*/
		const keyRoom = `room:${room}`;  // -> "{\"owner\":\"john\",\"participants\":[]}"
		const keyOwner = `user:${user}:owner`;

		try {
			// Check if room already exists
			const isRoomExists = await existsAsync(keyRoom);
			console.log(`Room ${room} already exists`);


			if (isRoomExists === 1) {
				// Check if user is owner of the room
				const isUserOwner = await sismemberAsync(keyOwner, room);

				if (isUserOwner === 1) {
					console.log(`User ${user} is owner of the room ${room}`);
					return {
						success: true,
						message: `User ${user} is owner of the room ${room}`,
						is_owner: true,
						room: room,
					};
				} else {
					console.log(`User ${user} is not owner of the room ${room}`);
					return {
						success: true,
						message: `User ${user} is not owner of the room ${room}`,
						is_owner: false,
						room: room,
					};
				}
			} else {
				console.log(`Room ${room} does not exist. Storing the room...`);
				const roomDetails = {
					owner: user,
					participants: [],
				};

				// Store the room details
				const isRoomStored = await setAsync(keyRoom, JSON.stringify(roomDetails));
				if (isRoomStored === 0) {
					console.warn(`Warning: room ${room} not added as expected...`);
				}
				console.log(`Room ${room} successfully stored`);

				// Add room in keyOwner
				const isUserSetAsOwner = await saddAsync(keyOwner, room);
				if (isUserSetAsOwner === 0) {
					console.warn(`Warning: User ${user} not added as owner of the room ${room} as expected...`);
				}
				console.log(`User ${user} successfully added as owner of the room ${room}`);

				return {
					success: true,
					message: `Room ${room} successfully stored with user ${user} as its owner.`,
					is_owner: true,
				};
			}
		} catch (error) {
			console.error(`Error processing room details for ${room}:`, error);
			return {
				success: false,
				message: `Failed to process room details`,
				error: error.message || error,
			};
		}
	},
	setRoomUserRelationship: function (room, user, callback) {
		const keyAllRooms = "scrumblr-rooms";
		const keyOwner = `user:${user}:owner`;

		// check if room already exists
		redisClient.sismember(keyAllRooms, room, (error, resAllRooms) => {
			if (error) {
				console.error(`Error checking existence for room ${room}:`, error);
				return callback({ success: false, message: 'Checking room existence failed', error: error });
			}

			if (resAllRooms === 1) {  // Room already exists (1 means the member exists)

				// check if user is owner of the room
				redisClient.sismember(keyOwner, room, (error, resOwner) => {
					if (error) {
						console.error(`Error retrieving room ${room} for user/owner ${user}:`, error);
						return callback({ success: false, message: `Error retrieving room ${room} for user/owner ${user}:`, error: error });
					}

					if (resOwner === 1) {  // Room exists in keyOwner
						console.log(`Room ${room} already stored ${keyOwner}`);

						return callback({
							success: true,
							message: 'Room already has an owner... Owner retrieved successfully',
							is_owner: true,
							room: room,
						});
					}
					else {
						console.log(`User ${user} is not the room ${room}'s owner`);
						return callback({
							success: true,
							message: 'Room already has an owner... Owner retrieved successfully',
							is_owner: false,
							room: room,
						});
					}
				})
			}
			else {  // If the room does not exist, add it to rooms and user/owner rooms
				redisClient.sadd(keyAllRooms, room, (error, resAddToRooms) => {
					if (error) {
						console.error(`Error storing ${room} in ${keyAllRooms}: `, error);
						return callback({ success: false, message: `Error storing ${room} in ${keyAllRooms}`, error: error });
					}
					console.log(`Successful storage of room "${room}" in ${keyAllRooms}`);

					redisClient.sadd(keyOwner, room, (error, resAddRoomToUser) => {
						if (error) {
							console.error(`Error storing ${room} in ${keyOwner}: `, error);
							return callback({ success: false, message: `Error storing ${room} in ${keyOwner}`, error: error });
						}
						console.log(`Successful storage of room "${room}" in ${keyOwner}`);

						return callback({
							success: true,
							message: `Room-user relationship successfully set`,
							is_owner: true
						});
					});
				});
			}
		});
	},
	getRoomOwner: function (room, callback) {
		// Check if the room already has an owner
		redisClient.exists(REDIS_PREFIX + '-room:' + room + '-owner', (error, response) => {
			if (error) {
				console.error(`Error checking existence for room ${room}:`, error);
				callback({ success: false, message: 'Checking room existence failed', error: error });
				return;
			}

			if (response !== 1) {  // Room 
				console.log(`Room ${room} does not exist yet.`);
				callback({ success: false, message: 'Room does not exist yet' });
				return;
			}

			redisClient.get(REDIS_PREFIX + '-room:' + room + '-owner', (error, response) => {
				if (error) {
					console.error(`Error retrieving owner for room ${room}:`, error);
					callback({ success: false, message: 'Retrieving room owner failed', error: error });
					return;
				}
				if (response === null) {
					console.log(`Owner for room ${room} not found.`);
					callback({ success: false, message: 'Owner not found' });
				} else {
					console.log(`Retrieved owner for room ${room}: ${response}`);
					callback({ success: true, message: 'Owner retrieved successfully', owner: response });
				}
			});
		});
	},
	addRoomToUser: function (user, room, role, callback) {  // type is either 'owner' or 'participant'
		const key = `user:${user}:${role}`;
		redisClient.sadd(key, room, (error, response) => {
			if (error) {
				console.error(`Error adding room to user with role set as ${role}:`, error);
				return callback({ success: false, message: `Error adding room to user with role set as ${role}:`, error: error });
			}

			if (response === 1) {
				console.log(`Room successfully added to user with role set as ${role}`);
				return callback({ success: true, message: `Room successfully added to user with role set as ${role}` });
			} else {
				console.log(`Room was already present for user with role set as ${role}`);
				return callback({ success: true, message: `Room was already present for user with role set as ${role}` });
			}
		})
	},
	addRoomToUserAsParticipant: async function (user, room) {
		const keyParticipant = `user:${user}:participant`;
		const keyOwner = `user:${user}:owner`;
		const keyRoom = `room:${room}`;

		try {
			const resOwner = await sismemberAsync(keyOwner, room);
			if (resOwner === 1) {
				let message = `Room ${room} already stored in ${keyOwner}. It will not be added in ${keyParticipant}.`
				console.log(message);
				return {
					success: true,
					message: message
				};
			}

			const response = await saddAsync(keyParticipant, room);
			let message = response === 1 ? `Room successfully added to ${keyParticipant}` : `Room was already present in ${keyParticipant}.`;
			console.log(message);

			// Handling room details
			const roomDetailsRaw = await getAsync(keyRoom);
			if (roomDetailsRaw) {
				let roomDetails = JSON.parse(roomDetailsRaw);

				if (!roomDetails.participants.includes(user)) {
					roomDetails.participants.push(user);
					await setAsync(keyRoom, JSON.stringify(roomDetails));
					console.log('Participant added successfully to room:', room);
					return { success: true, message: `Participant added successfully in ${keyRoom}` };
				}
			}
			return { success: true, message: message };
		} catch (error) {
			console.error(`Error processing room addition for user ${user}:`, error);
			return { success: false, message: `Error processing room addition for user ${user}`, error: error };
		}
	},
	getRoomParticipants: async function (room) {
		const keyRoom = `room:${room}`;
		try {
			const roomDetailsRaw = await getAsync(keyRoom);
			if (!roomDetailsRaw) {
				console.log(`No room details found for ${room}`);
				return [];  // Returns an empty array if no room details are found.
			}

			const roomDetails = JSON.parse(roomDetailsRaw);
			return roomDetails.participants || [];  // Safeguard in case there are no participants key.
		} catch (error) {
			console.error(`Error retrieving participants for room ${room}:`, error);
			throw new Error(`Unable to retrieve participants: ${error.message}`);
		}
	},
	getUserRooms: async function (user) {
		try {
			const keyParticipant = `user:${user}:participant`;
			const keyOwner = `user:${user}:owner`;

			// Retrieve room lists in parallel
			const [participantMembers, ownerMembers] = await Promise.all([
				smembersAsync(keyParticipant),
				smembersAsync(keyOwner)
			]);

			// Prepare owner room data with password fetching
			const ownerData = await Promise.all(ownerMembers.map(async room => {
				const password = await this._getPasswordAsync(room);
				return {
					room: room,
					role: 'owner',
					password: password ? Buffer.from(password, 'base64').toString() : ''
				};
			}));



			// Prepare participant room data
			const participantData = await Promise.all(participantMembers.map(async room => {
				const password = await this._getPasswordAsync(room);
				return {
					room: room,
					role: 'participant',
					password: password ? Buffer.from(password, 'base64').toString() : ''
				};
			}));

			// Prepare participant room data (no passwords)
			// const participantData = participantMembers.map(room => ({
			// 	room: room,
			// 	role: 'participant',
			// 	password: ''
			// }));

			// Combine all room data
			const allRoomsData = ownerData.concat(participantData);
			return { success: true, message: "Successfully retrieved rooms for user.", data: allRoomsData };

		} catch (error) {
			console.error(`Error retrieving or processing rooms for user ${user}:`, error);
			return { success: false, message: `Error retrieving or processing rooms for user ${user}`, error };
		}
	},
	deletePublicRoom: async (admin, room) => {
		try {
			// Check if admin is among admin users

			// delete room and all its dependent data
			
		} catch (error) {
			
		}

	},
	deleteUserRoom: async function (user, room) {
		const keyRoom = `room:${room}`;
		const keyOwner = `user:${user}:owner`;
		const keyRoomContent = `${REDIS_PREFIX}-room:${room}-*`
		let message = '';

		try {
			// check if user is owner ; only room's owner or admin are allowed to delete a room
			const isOwner = await sismemberAsync(keyOwner, room);
			console.log('Membership result:', isOwner);

			if (!isOwner) {
				message = `User ${user} is not allowed to delete the room ${room}.`
				console.log(message);
				return { success: false, message: message };
			}

			// get room participants and remove room in their related key
			const roomParticipants = await db.getRoomParticipants(room);
			console.log(`Participants in room ${room}: ${roomParticipants}`);

			let countUpdated = 0;

			for (const userParticipant of roomParticipants) {
				const keyParticipant = `user:${userParticipant}:participant`;
				console.log("keyParticipant:", keyParticipant);
				// Remove the room from the participant's set
				var result = await sremAsync(keyParticipant, room);
				if (result > 0) {
					console.log(`Room ${room} removed from ${keyParticipant}.`);
					countUpdated++;
				}
			}

			// remove room in the owner related key
			result = await sremAsync(keyOwner, room);
			if (result > 0) {
				console.log(`Room ${room} removed from ${keyOwner}.`);
			} else {
				console.log(`WARNING: Room ${room} not removed from ${keyOwner}.`);
			}


			// delete keyRoom redis key
			result = await delAsync(keyRoom);
			if (result > 0) {
				console.log(`${keyRoom} redis key removed.`);
			} else {
				console.log(`WARNING: ${keyRoom} redis key not removed.`);
			}

			// delete all redis key matching keyRoomContent pattern
			result = db._deleteRedisKeys(keyRoomContent);
			if (result > 0) {
				console.log(`All redis key matching pattern ${keyRoomContent} removed.`);
			} else {
				console.log(`WARNING: redis key matching pattern ${keyRoomContent} not removed.`);
			}

			return { success: true, message: `Room ${room} successfully removed` };
		} catch (error) {
			console.error(`Error removing the room: ${error}`);
		}
	},
	_deleteRedisKeys: function (pattern) {
		let cursor = '0';

		function scanAndDelete(cursor) {
			redisClient.scan(cursor, 'MATCH', pattern, 'COUNT', '10', (err, reply) => {
				if (err) {
					console.error("Error scanning keys:", err);
					return 0;
				}

				cursor = reply[0];
				const keys = reply[1];

				if (keys.length > 0) {
					redisClient.del(keys, (err, response) => {
						if (err) {
							console.error("Error deleting keys:", err);
							return;
						}
						console.log(`Deleted ${response} keys.`);
					});
				}

				// If cursor is not '0', continue scanning
				if (cursor !== '0') {
					scanAndDelete(cursor);
				} else {
					console.log("Scan complete.");
				}
			});
		}

		// Start scanning
		scanAndDelete(cursor);
		return 1;
	},
	clearRoom: function (room, callback) {
		redisClient.del(REDIS_PREFIX + '-room:/demo-cards', function (err, res) {
			redisClient.del(REDIS_PREFIX + '-room:/demo-columns', function (err, res) {
				callback();
			});
		});
	},
	_hashPassword: function (password) {
		return crypto.createHash('sha256').update(password).digest('hex');
	},
	setRoomPrivacy: async function (room, password) {
		try {
			const hashedPassword = this._hashPassword(password);
			const key = `${REDIS_PREFIX}-room:${room}-meta`;

			const response = await hsetAsync(key, 'hashedPwd', hashedPassword);
			if (response === 1) {
				console.log(`Password successfully set to key ${key}`);
			} else if (response === 0) {
				console.log(`New Password set to key ${key}`);
			}
		} catch (error) {
			console.error(`Error setting password for key ${key}: ${error}`);
		}
	},
	delRoomPrivacy: async function (room) {
		try {
			const previousPwd = await this.getRoomPassword(room);  // Get previous password if exists
			const keyAllowedUsers = `${REDIS_PREFIX}-room:${room}-allowed-users-${previousPwd}`;  // set room-allowed members key			

			var response = await this._delRedisKey(keyAllowedUsers);  // delete room-allowed members key
			if (response) {
				console.log(`Room-allowed members key successfully deleted: ${keyAllowedUsers}`);
			} else {
				console.log(`No key found for deletion: ${keyAllowedUsers}`);
			}


			const key = `${REDIS_PREFIX}-room:${room}-meta`;
			const field = 'hashedPwd';
			response = await hdelAsync(key, field);
			if (response) {
				console.log(`Password successfully deleted from key ${key}`);
			} else {
				console.log(`No key found for deletion: ${key}`);
			}

		} catch (error) {
			console.error(`Error deleting password for key ${key}: ${error}`);
		}
	},
	getRoomPassword: async function (room) {
		try {
			const key = `${REDIS_PREFIX}-room:${room}-meta`;
			const field = 'hashedPwd';
			const hashedPwd = await hgetAsync(key, field);
			if (hashedPwd) {
				console.log(`Password successfully retrieved from key ${key}`);
			} else {
				console.log(`No password found from key ${key}`);
			}
			return hashedPwd;  // null value
		} catch (error) {
			console.error(`Error retrieving password for key ${key}: ${error}`);
		}
	},
	setAllowedUsers: async function (room, username, hashedPassword) {
		try {
			const hashedPwd = await this.getRoomPassword(room);
			const key = `${REDIS_PREFIX}-room:${room}-allowed-users-${hashedPwd}`;
			const response = await saddAsync(key, username);
			if (response) {
				console.log(`User ${username} now allowed to access locked room ${room}`);
			} else {
				console.log(`User ${username} already allowed to access locked room ${room}`);
			}
		} catch (error) {
			console.error(`Error allowing user ${username} in locked room ${room}: ${error}`);
		}
	},
	isUserAllowed: async function (room, username) {
		try {
			const hashedPwd = await this.getRoomPassword(room);
			const key = `${REDIS_PREFIX}-room:${room}-allowed-users-${hashedPwd}`;
			const member = username;
			const isAllowed = await sismemberAsync(key, member);
			if (isAllowed) {
				console.log(`User ${username} is allowed to access locked room ${room}`);
			} else {
				console.log(`User ${username} is not allowed to access locked room ${room}`);
			}
			return isAllowed;  // 1 if true, 0 otherwise
		} catch (error) {
			console.error(`Error checking right of user ${username} to access locked room ${room}: ${error}`);
		}
	},
	setPassword: function (room, data) {
		redisClient.SET(REDIS_PREFIX + "-room:" + room + "-password", data, (err, res) => {
			if (err) {
				console.error("Error setting password:", err);
			}
		});
	},
	_isRedisKey: async function (key) {
		try {
			const isKeyExist = await existsAsync(key);
			return isKeyExist;
		} catch (error) {
			console.error(`Error in checking existence of key ${key}: ${err}`);
		}
	},
	_delRedisKey: async function (key) {
		try {
			const delResponse = await delAsync(key);
			return delResponse;
		} catch (error) {
			console.error(`Error in deleting key ${key}: ${err}`);
		}
	},
	_getPasswordAsync: async function (room) {
		try {
			const key = `${REDIS_PREFIX}-room:${room}-password`;
			const res = await getAsync(key);
			return res || '';
		} catch (err) {
			console.error("Error retrieving password for room:", room, err);
			return '';
		}
	},
	clearPassword: function (room) {
		redisClient.DEL(REDIS_PREFIX + "-room:" + room + "-password", (err, res) => {
			if (err) {
				console.error("Error clearing password:", err);
			}
		});
	},
	setFont: function (room, font) {
		redisClient.SET(REDIS_PREFIX + "-room:" + room + "-font", JSON.stringify(font), (err, res) => {
			if (err) {
				console.error("Error setting font:", err);
			}
		});
	},
	getFont: function (room, callback) {
		redisClient.GET(
			REDIS_PREFIX + "-room:" + room + "-font",
			function (err, data) {
				if (err) {
					console.error("Error setting font:", err);
				}

				callback(JSON.parse(data));
			});
	},
	setTheme: function (room, theme) {
		redisClient.set(REDIS_PREFIX + '-room:' + room + '-theme', theme);
	},
	getTheme: function (room, callback) {
		redisClient.get(REDIS_PREFIX + '-room:' + room + '-theme', function (err, res) {
			callback(res);
		});
	},
	createColumn: function (room, name, callback) {
		redisClient.rpush(REDIS_PREFIX + '-room:' + room + '-columns', name,
			function (err, res) {
				if (typeof callback != "undefined" && callback !== null) callback();
			}
		);
	},
	getAllColumns: function (room, callback) {
		redisClient.lrange(REDIS_PREFIX + '-room:' + room + '-columns', 0, -1, function (err, res) {
			callback(res);
		});
	},
	deleteColumn: function (room) {
		redisClient.rpop(REDIS_PREFIX + '-room:' + room + '-columns');
	},
	setColumns: function (room, columns) {
		//1. first delete all columns
		redisClient.del(REDIS_PREFIX + '-room:' + room + '-columns', function () {
			//2. now add columns for each thingy
			async.forEachSeries(
				columns,
				function (item, callback) {
					//console.log('rpush: ' + REDIS_PREFIX + '-room:' + room + '-columns' + ' -- ' + item);
					redisClient.rpush(REDIS_PREFIX + '-room:' + room + '-columns', item,
						function (err, res) {
							callback();
						}
					);
				},
				function () {
					//this happens when the series is complete
				}
			);
		});
	},
	createCard: function (room, id, card) {
		var cardString = JSON.stringify(card);
		redisClient.hset(`${REDIS_PREFIX}-room:${room}-cards`, id, cardString);
	},
	getAllCards: function (room, callback) {
		redisClient.hgetall(REDIS_PREFIX + '-room:' + room + '-cards', function (err, res) {
			var cards = [];

			for (var i in res) {
				cards.push(JSON.parse(res[i]));
			}

			callback(cards);
		});
	},
	cardEdit: function (room, id, text) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', id, function (err, res) {
			var card = JSON.parse(res);
			if (card !== null) {
				card.text = text;
				console.log(JSON.stringify(card));
				redisClient.hset(`${REDIS_PREFIX}-room:${room}-cards`, id, JSON.stringify(card));
			}
		});
	},
	cardSetXY: function (room, id, x, y) {
		redisClient.hget(`${REDIS_PREFIX}-room:${room}-cards`, id, function (err, res) {
			var card = JSON.parse(res);
			if (card !== null) {
				card.x = x;
				card.y = y;
				redisClient.hset(`${REDIS_PREFIX}-room:${room}-cards`, id, JSON.stringify(card));
			}
		});
	},
	deleteCard: function (room, id) {
		redisClient.hdel(`${REDIS_PREFIX}-room:${room}-cards`, id);
	},
	changeCardColor: async function (room, cardId, color) {
		try {
			const key = `${REDIS_PREFIX}-room:${room}-cards`;
			const res = await hgetAsync(key, cardId);
			let card = JSON.parse(res);

			if (card !== null) {
				// Update the color of the card
				card.colour = color; // Set the new color

				// Serialize the updated card object back to a JSON string
				const updatedCardJson = JSON.stringify(card);

				// Save the updated card back into Redis
				await hsetAsync(key, cardId, updatedCardJson);

				console.log(`Card color updated successfully: ${cardId}`);
				return // { success: true, message: "Card color updated successfully." };
			} else {
				console.log(`Card not found: ${cardId}`);
				return // { success: false, message: "Card not found." };
			}
		} catch (err) {
			console.error("Error in changeCardColor function:", err);
			return { success: false, message: "Error updating card color.", error: err };
		}
	},
	updateSticker: async function (room, cardId, stickerId, operation) {
		try {
			const key = `${REDIS_PREFIX}-room:${room}-cards`;
			const res = await hgetAsync(key, cardId);
			let card = JSON.parse(res);

			if (card !== null) {
				let stickerSet = new sets.Set(card.sticker || []);

				if (operation === 'add') {
					stickerSet.add(stickerId);
				} else if (operation === 'remove') {
					stickerSet.remove(stickerId);
				}

				card.sticker = stickerSet.array();
				await hsetAsync(key, cardId, JSON.stringify(card));
			}
		} catch (err) {
			console.error("Error in updateSticker function:", err);
		}
	},
	setBoardSize: function (room, size) {
		redisClient.set(REDIS_PREFIX + '-room:' + room + '-size', JSON.stringify(size));
	},
	getBoardSize: function (room, callback) {
		redisClient.get(REDIS_PREFIX + '-room:' + room + '-size', function (err, res) {
			callback(JSON.parse(res));
		});
	}

};



function createCard(room, id, text, x, y, rot, colour) {
	var card = {
		id: id,
		colour: colour,
		rot: rot,
		x: x,
		y: y,
		text: text,
		sticker: null,
		user: "demo-user"
	};

	db.createCard(room, id, card);
}

function roundRand(max) {
	return Math.floor(Math.random() * max);
}

function cleanAndInitializeDemoRoom() {
	console.log('Initializing demo room...');
	// DUMMY DATA
	db.clearRoom('demo', function () {
		db.createColumn('demo', 'Not Started');
		db.createColumn('demo', 'Started');
		db.createColumn('demo', 'Testing');
		db.createColumn('demo', 'Review');
		db.createColumn('demo', 'Complete');


		createCard('demo', 'card1', 'Hello this is fun', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'yellow');
		createCard('demo', 'card2', 'Hello this is a new story.', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'white');
		createCard('demo', 'card3', '.', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'blue');
		createCard('demo', 'card4', '.', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'green');

		createCard('demo', 'card5', 'Hello this is fun', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'yellow');
		createCard('demo', 'card6', 'Hello this is a new card.', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'yellow');
		createCard('demo', 'card7', '.', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'blue');
		createCard('demo', 'card8', '.', roundRand(600), roundRand(300), Math.random() * 10 - 5, 'green');
	});
}


function generateToken() {
	return crypto.randomBytes(20).toString('hex');  // Generates a 40-character hexadecimal token
}

// exports.db = db;
module.exports = { db, redisClient };
