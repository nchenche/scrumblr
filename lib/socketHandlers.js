        ...
        client.on('message', function (message) {
            ...
            switch (message.action) {
                ...
                case 'joinRoom':
                    ...
                    client.join(room);  // Join the client to the room
                    if (!roomUsers[room]) {
                        roomUsers[room] = new Set();  // Initialize the set if it doesn't exist
                    }
                    roomUsers[room].add(username);  // Add user to the room
            
                    updateRoomUsers(room);  // Emit updated user list to the room
...
function initClient(client, db) {
    ...
    getRoom(client, async function (room) {
        ...
        try {
            ...
            client.json.send({
                action: 'error',
                message: 'Failed to retrieve room password.'
            });
        };
    });
}
...
function joinRoom(client, data, callback) {
    var msg = {
